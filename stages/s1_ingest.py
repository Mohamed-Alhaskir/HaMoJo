...
class DataIngestionStage(BaseStage):
    ...
    def run(self, ctx: dict) -> dict:
        ...
        # ---- Load per-session overrides from session_overview.json ----
        # session_overview.json is the durable source of truth for per-session
        # values that don't live in metadata: which quadrant to analyse, and
        # the conversation start/end window (used to crop audio for ASR).
        # Falls back silently if the file or session entry is missing.
        overview_entry = self._load_overview_entry(ctx.get("session_id", ""))

        # ---- Resolve which quadrant Stage 04 should analyse ----
        # Priority: session_overview > metadata > config default
        video_analysis_quadrant = (
            overview_entry.get("video_analysis_quadrant")
            or metadata.get("video_analysis_quadrant")
            or composite_cfg.get("video_analysis_quadrant", "bottom_left")
        )

        # ---- Resolve conversation window (defaults to full audio) ----
        conversation_start_s = overview_entry.get("conversation_start_s", 0.0)
        conversation_end_s   = overview_entry.get("conversation_end_s",   duration)
        ...
        inventory = {
            ...
            "audio": {
                "path": str(audio_path),
                "duration_s": duration,
                "conversation_start_s": conversation_start_s,
                "conversation_end_s": conversation_end_s,
            },
            "quadrants": quadrants,
    ...
    # ------------------------------------------------------------------
    # Per-session overrides from session_overview.json
    # ------------------------------------------------------------------

    def _load_overview_entry(self, session_id: str) -> dict:
        """
        Look up the per-session entry in session_overview.json (project root).
        Returns an empty dict if the file or the entry is missing — the caller
        falls back to defaults so a missing overview file does not break ingest.
        """
        if not session_id:
            return {}
        overview_path = Path("session_overview.json")
        ...
        try:
            ...
            if entry:
                self.logger.info(
                    f"Loaded session_overview entry for '{session_id}': "
                    f"quadrant={entry.get('video_analysis_quadrant')}, "
                    f"conv=[{entry.get('conversation_start_s')}, {entry.get('conversation_end_s')}]"
                )
            return entry
        except (json.JSONDecodeError, OSError) as exc:
            self.logger.warning(
                f"Could not read session_overview.json: {exc} — using defaults."
            )
            return {}
    ...
    def _load_metadata(
        ...
        self.logger.warning(
            ...
            "auto-generating; scenario.id will be filled in by "
            "_ensure_scenario_id() from session_overview.json."
        )
    ...
    def _ensure_scenario_id(self, metadata: dict, ctx: dict) -> None:
        ...
        Resolution priority:
          1. session_overview.json (project root) — durable per-session source of truth
          2. existing metadata.scenario.id (user-provided or auto-generated)
          3. utils.scenario_map.resolve_scenario_id() — legacy session_scenario_map.json

        session_overview takes precedence over an existing metadata value so that
        edits to session_overview.json propagate on the next ingest run, instead
        of being silently overridden by a stale metadata file on disk.
        """
        session_id = ctx.get("session_id", "")

        # 1. session_overview.json
        overview_entry = self._load_overview_entry(session_id)
        overview_scenario = (overview_entry.get("scenario") or "").strip()
        if overview_scenario:
            previous = metadata.get("scenario", {}).get("id", "").strip()
            metadata.setdefault("scenario", {})
            metadata["scenario"]["id"] = overview_scenario
            if previous and previous != overview_scenario:
                self.logger.info(
                    f"scenario_id from session_overview overrides existing "
                    f"metadata value: '{previous}' → '{overview_scenario}'"
                )
            else:
                self.logger.info(
                    f"scenario_id from session_overview: "
                    f"'{session_id}' → '{overview_scenario}'"
                )
            return

        # 2. Already present in metadata
        ...
        # 3. Legacy session_scenario_map.json fallback
    ...
    def _auto_generate_metadata(self, input_dir: Path, ctx: dict) -> dict:
        ...
        # Note: scenario.id is resolved later in _ensure_scenario_id() which
        # consults session_overview.json as the durable source of truth. We
        # intentionally leave scenario.id empty here.
