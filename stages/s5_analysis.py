...
======================================
Generic instrument loop driven by the ``scenarios`` block in
``session_overview.json`` (project root).
...
def _load_scenario_definitions() -> dict[str, dict]:
    """Load the ``scenarios`` block from ``session_overview.json``.

    This replaces the legacy ``templates/scenario_catalog.json`` — all
    scenario → instrument mappings now live in session_overview.json.
    """
    overview_path = Path(__file__).parent.parent / "session_overview.json"
    ...
    try:
        ...
        print(f"⚠ Error loading session_overview.json: {e}")
        return {}
...
class LLMAnalysisStage(BaseStage):
    ...
    def run(self, ctx: dict) -> dict:
        ...
        scenario_definitions = _load_scenario_definitions()
        scenario_cfg = scenario_definitions.get(scenario_id, _DEFAULT_SCENARIO_CONFIG)
