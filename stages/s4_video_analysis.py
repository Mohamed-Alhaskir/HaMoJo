...
class VideoAnalysisStage(BaseStage):
    ...
    def _resolve_video_source(self, ctx, cfg):
        ...
        # Priority: inventory setting > config fallback > composite video
        # Per-session inventory is the source of truth; config is only the
        # default applied when a session has no quadrant specified.
        ...
        if inventory_quadrant:
            ...
            self.logger.warning(
                ...
                f"in quadrants {list(quadrants.keys())}; falling back to config."
            )

        preferred_quadrant = cfg.get("preferred_quadrant")
        if preferred_quadrant and preferred_quadrant in quadrants:
            self.logger.info(f"Video analysis quadrant from config fallback: '{preferred_quadrant}'")
